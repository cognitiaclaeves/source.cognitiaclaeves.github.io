#!/bin/bash

# I wanted to maintain light & dark themes for the site, and to use git/jekyll for the site
# To do this, resorted to 'pre-processing' files to duplicate files and swap themes statically
# ( and 'post-processing' to duplicate paginated index.html to index-dark.html, and to swap in theme-switch URLs. )

# Remove all alternate-theme files and create directories again
# ( Currently supporting two themes: light (original), and dark

for dir in dark archives/dark _posts/dark; do
  echo -e "\nCulling $dir:"
  rm -rv "$dir" | grep '\.'
  mkdir "$dir" # Make directory again ... ( there may be a better than these two commands ) 
done

echo
echo 'Copying working files to new theme(s):'
echo
cp -av light/* dark
cp -av archives/light/* archives/dark
cp -av _posts/light/* _posts/dark



echo
echo "Sed'ing YAML in new files:"
echo
# Sed-switch files in new theme(s) to match yaml of new theme(s)
for dir in dark archives/dark _posts/dark; do
  echo "Sed'ing files in $dir."
  # switch custom 'theme' attribute:
  find "$dir" -type f -exec sed -i '' 's/theme: light/theme: dark/g' {} +
  # switch all possible layouts
  find "$dir" -type f -exec sed -i '' 's/layout: default_light/layout: default_dark/g' {} +
  find "$dir" -type f -exec sed -i '' 's/layout: post/layout: post_dark/g' {} +

  # switch internal links
  find "$dir" -type f -exec sed -i '' 's#post_url /light/#post_url /dark/#g' {} +
  # any urls referencing tag-summary pages
  find "$dir" -type f -exec sed -i '' 's#tags/light#tags/dark#g' {} +
done

echo
# switch 'categories: light' to new theme(s)
# very hacky ... to avoid needing to specify permalinks for every post
find "$dir" -type f -exec sed -i '' 's/categories: light/categories: dark/g' {} +

# It turns out that 'pre-processing' and 'post-processing' need to be run at the same time:
# Anytime a change is made prior to rendering a page on the site.

sleep 8

echo


# One directive that is important to me with blogging is that I not spend too much time on maintaining it.
# This script is here to 'patch' anything that happens with Jekyll that I don't like.
# While I try to find the 'right' ways to do things, I also think it's important not to
#   exhaust the time I have for non income generating projects.

# The pagination plug-in for Jekyll may only support pagination for one index page per site.
# This still makes things interesting when supporting static theme-switching while using friendly URLs.

#
# TODO: How much of this can be handled before generation??
#

echo "Running post-process hacks."

# Handle pagination links
cp -av _site/page* _site/light
cp -av _site/page* _site/dark

cp _site/index.html _site/light/index.html
cp _site/index.html _site/dark/index.html

find _site/light -type f -print0 | xargs -0 sed -i '' 's#href="/page#href="/light/page#g'
find _site/dark -type f -print0 | xargs -0 sed -i '' 's#href="/page#href="/dark/page#g'

# Special case: Don't let 2nd page of dark reference root of light
sed -i .bak 's#class="prev left" href="/"#class="prev left" href="/dark"#g' '_site/dark/page2/index.html'

#find _site/light -type f -print0 | xargs -0 sed -i '' 's#href="/#href="/light/#g'
#find _site/dark -type f -print0 | xargs -0 sed -i '' 's#href="/#href="/dark/#g'

# Tags are generated after Jekyll processes ... 
mkdir _site/tags
cp -av _site/tag _site/tags/light
cp -av _site/tag _site/tags/dark
rm -rf _site/tag

# Force-fit dark side
find _site/dark -type f -print0 | xargs -0 sed -i '' 's#colors-light#colors-dark#g'
find _site/dark -type f -print0 | xargs -0 sed -i '' 's#href="/">Home#href="/dark">Home#g'
find _site/dark -type f -print0 | xargs -0 sed -i '' 's#/light"#/dark"#g'
find _site/dark -type f -print0 | xargs -0 sed -i '' 's#/light/#/dark/#g'

#sed -i .bak 's#colors-light#colors-dark#g' '_site/dark/index.html'
#sed -i .bak 's#href="/">Home#href="/dark">Home#g' '_site/dark/index.html'
#sed -i .bak 's#/light"#/dark"#g' '_site/dark/index.html'
#sed -i .bak 's#/light/#/dark/#g' '_site/dark/index.html'


# Force fit tags:
# Tags are all generated by Jekyll
# So they are mangled a little differently

# ... Into index pages

find _site/light -type f -print0 | xargs -0 sed -i '' 's#/tag#/tags/light#g'
find _site/dark -type f -print0 | xargs -0 sed -i '' 's#/tag#/tags/dark#g'

#sed -i .bak 's#/tag#/tags/light#g' '_site/light/index.html'
#sed -i .bak 's#/tag#/tags/dark#g' '_site/dark/index.html'

# Main index page
sed -i .bak 's#/tag#/tags/light#g' '_site/index.html'

# Archive pages
find _site/archives/light -type f -print0 | xargs -0 sed -i '' 's#/tag#/tags/light#g'
find _site/archives/dark -type f -print0 | xargs -0 sed -i '' 's#/tag#/tags/dark#g'

#sed -i .bak 's#/tag#/tags/dark#g' '_site/archives/dark/index.html'
#sed -i .bak 's#/tag#/tags/light#g' '_site/archives/light/index.html'

# Tag pages
find _site/tags/light -type f -print0 | xargs -0 sed -i '' 's#/tag/#/tags/light/#g'
find _site/tags/dark -type f -print0 | xargs -0 sed -i '' 's#/tag/#/tags/dark/#g'

find _site/tags/dark -type f -print0 | xargs -0 sed -i '' 's#/light/#/dark/#g'
find _site/tags/dark -type f -print0 | xargs -0 sed -i '' 's#/archives/light#/archives/dark#g'

# Change theme in dark

# colors-light.css" rel="stylesheet"
find _site/tags/dark -type f -print0 | xargs -0 sed -i '' 's#colors-light.css"#colors-dark.css"#g'

# Post pages
find _site/light -type f -print0 | xargs -0 sed -i '' 's#/tag/#/tags/light/#g'
find _site/dark -type f -print0 | xargs -0 sed -i '' 's#/tag/#/tags/dark/#g'


echo
echo 'Embedding theme-switching URLs...'
echo
./embed-theme-switch-URLs.py

echo

# After all the mangling is done, just copy the light version to the root
cp _site/light/index.html _site/index.html

